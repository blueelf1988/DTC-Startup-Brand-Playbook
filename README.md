...
| 场景 | 你只需说 | Skill 自动输出 |
|------|---------|--------------|
| 🆕 **提交新 idea** | "我想做一个面向北美的宠物用品 DTC 品牌……" | 完整 7 模块商业策划书（业态定位→GO/NO-GO→机会→战略→模型→GTM→RoadMap） |
| ⚡ **快速可行性判断** | "帮我看看这个方向能不能做：XX 品类出海" | 先给 GO/NO-GO 结论 + 5 维评估，再决定是否展开完整分析 |
| 🔍 **已有 idea 深挖** | "对 Idea #1 做商业模型和 Unit Economics 深挖" | 单独展开商业模型模块，含行业基准对比 |
| 💰 **融资 Pitch** | "帮我把 Idea #1 整理成投资人能看的版本" | 精简为 Executive Summary 风格（3 页以内） |
| ⚔️ **竞品对标** | "帮我分析 XX 赛道在欧美的竞争格局" | 四象限图 + 核心竞品深度拆解 |
| 🚀 **GTM 策略** | "帮我规划 XX 品牌的冷启动方案" | 5P 模型 + 内容-红人-投放三引擎 + 12 个月 RoadMap |
| 📊 **对比筛选** | "我有 3 个 idea，帮我横向对比选最优" | 横向对比矩阵 + 推荐排序 + 加权评分 |

### 触发关键词

说出以下任一关键词即自动触发：`DTC`、`品牌策划`、`创业idea`、`商业计划书`、`GTM`、`0-1`、`品牌出海`、`跨境电商`、`品类创建`、`冷启动`
